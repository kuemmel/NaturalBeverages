	...
	throws ServletException, IOException 
	{
		...
		//delete old tables, create new ones
			try 
			{
				...
				statement.executeUpdate("create table users (email varchar(64), nick varchar(20), password text, name varchar(20), surname varchar(20), dateofbirth date, address varchar(64), floor int(2), zipcode int(6), dateofsignup timestamp default current_timestamp on update current_timestamp, primary key (email)) engine = INNODB;");
				...
				statement.executeUpdate("create table orders (id mediumint not null unique auto_increment, email varchar(64), dateoforder timestamp default current_timestamp on update current_timestamp, service boolean, primary key (email,dateoforder), constraint foreign key (email) references dz39.users(email) on update restrict on delete cascade) engine = INNODB;");
